...
const class_name = ['main', 'xiajia', 'duimian'];
const feng_hanzi = ['東', '南', '西'];
const shu_hanzi = ['一', '二', '三'];
...
class Score {
    ...
    redraw(viewpoint) {
        ...
        for (let l = 0; l < 3; l++) {
            ...
            let c = class_name[(id + 3 - this._viewpoint) % 3];
            this._view.defen[l] = $(`.defen .${c}`, this._view.root);
...
module.exports = class Board {
    ...
    set_audio(audio) {
        this._audio = {};
        for (let name of ['dapai', 'peng', 'gang', 'rong', 'zimo', 'lizhi']) {
            this._audio[name] = [];
            for (let l = 0; l < 3; l++) {
    ...
    redraw() {
        ...
        for (let l = 0; l < 3; l++) {
            let id = this._model.player_id[l];
            let c = class_name[(id + 3 - this.viewpoint) % 3];

            const rawName = this._model.player && this._model.player[id];
            const safeName = typeof rawName === 'string' ? rawName.replace(/\n.*$/, '') : '';
            show($(`> .player.${c}`, this._root).text(safeName));
    ...
    kaiju(viewpoint) {
        ...
        for (let id = 0; id < 3; id++) {
            let c = class_name[(3 - this.viewpoint + id) % 3];
            const rawName = this._model.player && this._model.player[id];
