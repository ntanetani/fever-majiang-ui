...
function get_next_qipai(paipu, rule, log) {
    ...
    let game = board.zhuangfeng * 3 + board.jushu;
    if (!lianzhuang) game++;
    ...
    return {
        ...
        defen: defen.splice((paipu.qijia + game) % 3).concat(defen),
        baopai: '',
        shoupai: ['', '', '']
    };
}
...
function jieju(paipu, rule) {
    ...
    if (!paipu.defen.length) {
        ...
        defen = defen.splice((6 - paipu.qijia - qipai.jushu) % 3).concat(defen);
    }

    let paiming = [];
    for (let i = 0; i < 3; i++) {
        let id = (paipu.qijia + i) % 3;
        for (let j = 0; j < 3; j++) {
...
module.exports = class PaipuEditor {
    ...
    select_log(log_idx) {
        ...
        if (log_idx >= 0) {
            ...
            viewpoint = (this._paipu.qijia + qipai.jushu) % 3;
        }
    ...
    update_score() {
        ...
        qipai.zhuangfeng = $('[name="game"]', score).val() / 3 | 0;
        qipai.jushu = $('[name="game"]', score).val() % 3;
    ...
    draw_paipu() {
        ...
        for (let i = 0; i < log.length; i++) {
            ...
            if (data.qipai) {
                $('[name="game"]', score)
                    .val(data.qipai.zhuangfeng * 3 + data.qipai.jushu);
                ...
                for (let l = 0; l < 3; l++) {
                    $('.player .jia', paipu).eq(l)
                        .text(['東', '南', '西'][l]);
                    let id = (this._paipu.qijia + data.qipai.jushu + l) % 3;
    ...
    update_paipu() {
        ...
        let qipai = {
            zhuangfeng: $('[name="game"]', score).val() / 3 | 0,
            jushu: $('[name="game"]', score).val() % 3,
    ...
    make_hule(l, fubaopai, exist) {
        ...
        if (last.dapai || last.gang) {
            ...
            rongpai = rongpai + ['_', '+', '=', '-'][(3 + baojia - l) % 3];
    ...
    make_pingju() {
        ...
        let pingju = { name: '', shoupai: ['', '', ''], fenpei: [0, 0, 0] };
